...
class VideoUrlHyperlinkedIdentityField(serializers.HyperlinkedIdentityField):
    """
    VideoUrlHyperlinkedIdentityField class refers generic view
    'VideoDetailAPIView' that exists in videos/views.py
    """
...
class VideoSerializer(serializers.HyperlinkedModelSerializer):
    ...
    """
    # customized API getting a link to Video instance. it uses
    # VideoUrlHyperlinkedIdentityField class.
    ...
    # Taking all comments related to Video instance and putting
    # it into VideoSerializer model
    comment_set = CommentSerializer(many=True, read_only=True)

    # it serializes entire model into Video Serializers,
    # it prints out all fields in Category model
    # category = CategorySerializer(many=False, read_only=True)

    # category_title = serializers.CharField(source='category.title',
    # read_only=True)
    # category_image = serializers.CharField(source='category.get_image_url',
    # read_only=True)

    # PrimaryKeyRelatedField is needed for getting an access to
    # related field which is in this case Category Model
    ...
    class Meta:
        ...
        fields = ['url',  # it's URL for API itself not model's API
                  ...
                  # 'category_image',
                  'category_url',
                  'comment_set',  # _set will show any comments
                  ]


class VideoViewSet(viewsets.ModelViewSet):
    # VideoViewSet is depricated.
    # Take a look at generic views in videos/views.py
    """
...
class CategorySerializer(serializers.HyperlinkedModelSerializer):
    ...
    # Category instance. It uses reverse 'category_detail_api'
    # which exists in urls.py
    url = serializers.HyperlinkedIdentityField('category_detail_api',
                                               lookup_field='slug')

    # it serializes entire Video model into Category Serializer.
    # it prints out all fields from Videp model
